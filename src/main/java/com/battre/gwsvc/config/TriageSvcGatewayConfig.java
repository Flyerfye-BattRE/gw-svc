...
public class TriageSvcGatewayConfig {
    ...
    public RouteLocator triageSvcRouteLocator(RouteLocatorBuilder builder) {
        return builder.routes()
                ...
                .route("triagesvc_generateIntakeBatteryOrder", r -> r.path("/triage")
                        .and()
                        .method(HttpMethod.GET)
                        .filters(f -> f.filter((exchange, chain) -> {
                            return processNoInputGrpcRequest(exchange, chain, grpcMethodInvoker::generateIntakeBatteryOrder);
                        }))
                        .uri("no://op"))
                .build();
    }
